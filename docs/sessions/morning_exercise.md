...
Write a for-loop with the loop variable `i`, which runs from `0` to `len(A)`
and prints out `i`.
...
Write a short program which compares two variables i and j. It should
...
Chose two variables, e.g. A and B and assign the sequences `GATTACA` and
...
Now extend the program: Concatenate both sequences in both ways (`AB` and
...
Print out the second sequence from the last to the first position (last
...
Assign this inverted sequence to a third variable, you could use the
